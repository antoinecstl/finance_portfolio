    ...
    <div className="mx-auto max-w-5xl rounded-xl bg-white dark:bg-zinc-900 shadow-2xl" onClick={event => event.stopPropagation()}>
      ...
      <div className="grid lg:grid-cols-[360px_1fr] gap-6 p-4 sm:p-6">
        ...
        <section className="space-y-5">
          <div><label className="block text-sm font-medium mb-1" htmlFor="share-title">Message (facultatif)</label><input id="share-title" maxLength={100} value={settings.title ?? ''} onChange={e => patch('title', e.target.value)} placeholder="Une conviction de long terme…" className="w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-transparent px-3 py-2" /></div>
