...
function PositionRow({ position, quote }: PositionRowProps) {
  ...
  return (
    <tr className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors hidden sm:table-row">
      <td className="py-3 sm:py-4 px-3 sm:px-4">
        <div>
          <p className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm sm:text-base">
            {position.symbol}
          </p>
          <p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400 truncate max-w-[100px] sm:max-w-[150px]">
            {position.name}
          </p>
        </div>
      </td>
      ...
      <td className="py-3 sm:py-4 px-3 sm:px-4 text-right">
        <p className={`font-semibold text-sm sm:text-base ${isPositiveTotal ? 'text-emerald-600' : 'text-red-600'}`}>
          {formatCurrency(totalGain)}
        </p>
        <p className={`text-xs sm:text-sm ${isPositiveTotal ? 'text-emerald-600' : 'text-red-600'}`}>
...
// Version mobile de PositionRow (affichée en carte)
function PositionCardMobile({ position, quote }: PositionRowProps) {
  const currentPrice = quote?.price || position.current_price;
  const dayChange = quote?.change || 0;
  const dayChangePercent = quote?.changePercent || 0;
  ...
  return (
    <div className="sm:hidden border-b border-zinc-100 dark:border-zinc-800 p-3 space-y-2">
      <div className="flex items-start justify-between">
        ...
        <div className="text-right">
          ...
          <div className={`flex items-center justify-end gap-1 text-xs ${isPositiveTotal ? 'text-emerald-600' : 'text-red-600'}`}>
            {isPositiveTotal ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
            {formatCurrency(totalGain)} ({formatPercent(totalGainPercent)})
          </div>
        </div>
      </div>
      <div className="flex justify-between text-xs text-zinc-500 dark:text-zinc-400">
        <span>{formatNumber(position.quantity, position.quantity % 1 === 0 ? 0 : 2)} × {formatCurrency(position.average_price)}</span>
        <span className="flex items-center gap-1">
          {formatCurrency(currentPrice)}
          <span className={isPositiveDay ? 'text-emerald-600' : 'text-red-600'}>
            ({formatPercent(dayChangePercent)})
          </span>
        </span>
      </div>
    ...
    <div className="space-y-3 sm:space-y-4">
      ...
      {positions.length > 0 && (
        <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
          <div className="px-3 sm:px-4 py-2 sm:py-3 bg-zinc-50 dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-700">
          ...
          <div className="sm:hidden">
            ...
            <div className="p-3 bg-zinc-50 dark:bg-zinc-800/50 flex justify-between items-center">
              <span className="font-semibold text-sm text-zinc-900 dark:text-zinc-100">Total</span>
              <div className="text-right">
                <p className="font-bold text-sm text-zinc-900 dark:text-zinc-100">{formatCurrency(totalValue)}</p>
            ...
            <table className="w-full">
              ...
              <tfoot>
                <tr className="bg-zinc-50 dark:bg-zinc-800/50 font-semibold">
                  <td colSpan={4} className="py-3 sm:py-4 px-3 sm:px-4 text-sm text-zinc-900 dark:text-zinc-100">
                    Total positions
                  </td>
                  <td className="py-3 sm:py-4 px-3 sm:px-4 text-right text-sm text-zinc-900 dark:text-zinc-100">
                    {formatCurrency(totalValue)}
                  </td>
                  <td className={`py-3 sm:py-4 px-3 sm:px-4 text-right text-sm ${
