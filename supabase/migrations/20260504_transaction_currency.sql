...
--
-- Avant cette migration : amount/fees/price_per_unit étaient implicitement EUR.
-- Le LLM convertissait USD/GBP avec un "taux raisonnable" non auditable, ce qui
-- cassait l'import des relevés crypto/Binance (USDC) et toute action cotée USD.
--
...
--   { type: CONVERSION, amount: 1000, currency: EUR,
--     target_amount: 1085, target_currency: USDC, date: 2026-04-12 }
-- Implicit rate : 1.085 USDC/EUR (ce que t'a coûté l'opération).
...
alter table public.transactions
  add column if not exists target_amount numeric(24, 10);
...
alter table public.transactions
  ...
  alter column target_currency type varchar(10);
...
-- ---------------------------------------------------------------------------
-- 2. Contraintes de format ISO + de cohérence CONVERSION
-- ---------------------------------------------------------------------------
do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'transactions_currency_format'
  ) then
    alter table public.transactions
      add constraint transactions_currency_format
      check (currency ~ '^[A-Z]{3,10}$') not valid;
  end if;

  if not exists (
    select 1 from pg_constraint where conname = 'transactions_target_currency_format'
  ) then
    alter table public.transactions
      add constraint transactions_target_currency_format
      check (target_currency is null or target_currency ~ '^[A-Z]{3,10}$') not valid;
  end if;
end $$;

-- 3. Étend l'enum type pour accueillir CONVERSION.
do $$
begin
  if exists (
...
alter table public.transactions
  ...
  check (type in ('DEPOSIT', 'WITHDRAWAL', 'BUY', 'SELL', 'DIVIDEND', 'INTEREST', 'FEE', 'CONVERSION'));

-- 4. Une CONVERSION exige target_* non nuls et différents de la devise source.
do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'transactions_conversion_fields'
  ) then
    alter table public.transactions
      add constraint transactions_conversion_fields
      check (
        type <> 'CONVERSION'
        or (
          ...
          and target_currency <> currency
        )
      ) not valid;
  end if;

  -- Inversement : pour tous les autres types, target_* doivent être null.
  if not exists (
    select 1 from pg_constraint where conname = 'transactions_no_target_for_non_conversion'
  ) then
    alter table public.transactions
      add constraint transactions_no_target_for_non_conversion
      check (
        type = 'CONVERSION'
        or (target_amount is null and target_currency is null)
      ) not valid;
  end if;
end $$;

-- ---------------------------------------------------------------------------
-- 5. Validation de séquence devenue currency-aware.
-- Avant : un seul bucket cash. Maintenant : un bucket par devise. Une
-- CONVERSION débite source / crédite target. Chaque bucket doit rester >= 0.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 6. Rebuild de stock_position : la devise de la position vient désormais des
-- transactions BUY (et non plus 'EUR' codé en dur). Si aucune BUY, on retombe
-- sur la devise du compte.
-- ---------------------------------------------------------------------------
...
begin
  ...
  for tx in
    select type, coalesce(quantity, 0) as quantity,
  ...
  loop
    if tx.type = 'BUY' then
      ...
      -- La devise de la position suit les BUY (la dernière vue).
...
-- ---------------------------------------------------------------------------
-- 7. RPC insert_transaction_with_fee : drop+recreate avec p_currency, p_target_*
-- (l'ajout de paramètres à valeur par défaut change la signature, donc DROP).
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Ligne FEE liée si frais > 0 (sauf si le tx principal est lui-même FEE).
...
-- ---------------------------------------------------------------------------
-- 8. RPC update_transaction_with_fee : même traitement, drop+recreate avec
-- les nouveaux paramètres currency / target_*.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- p_currency est optionnel : si non fourni, on conserve la devise actuelle.
  ...
  -- Synchronisation de la ligne FEE liée (création/maj/suppression).
...
-- ---------------------------------------------------------------------------
-- 9. RPC insert_transactions_batch : signature inchangée, seul le lecteur
-- de jsonb apprend à reconnaître currency / target_amount / target_currency.
-- ---------------------------------------------------------------------------
