    ...
    transactions: {
      ...
      items: {
        ...
        properties: {
          ...
          stock_symbol: {
            type: ['string', 'null'],
            description: 'Symbole de marché pour BUY/SELL/DIVIDEND. Null sinon. Actions: .PA (Paris), .DE (Xetra), .L (Londres). Crypto: BTC-USD, ETH-USD, SOL-USD (jamais de paire stable -USDC, mappée sur -USD). Null pour CONVERSION.',
          },
...
- stock_symbol : pour BUY/SELL/DIVIDEND uniquement. Actions cotées (.PA, .DE, .L, .MI, .SW). Crypto : -USD (BTC-USD, ETH-USD, SOL-USD) — même si l'exchange affiche -USDC ou -USDT, mappe-le sur -USD car c'est le ticker standard de valorisation et signale la devise réelle dans currency. Null sinon (et toujours null pour CONVERSION).
...
- Le code ISIN (FR..., US..., NL..., DE..., LU..., IE...) figure souvent sur la ligne suivante du libellé. Convertis-le en ticker de marché que tu connais (ex: FR0000121014 → MC.PA, FR0000120271 → TTE.PA, NL0000235190 → AIR.PA, US5949181045 → MSFT, US0378331005 → AAPL). Si l'ISIN est inconnu, mets stock_symbol à null et signale-le dans notes.
...
- Colonnes "Pair / Market" type "SOLUSDC", "BTCUSDC" : BUY/SELL d'actif sur compte crypto. stock_symbol = ticker de valorisation (SOL-USD, BTC-USD), currency = devise réelle (USDC). amount/price_per_unit en USDC.
